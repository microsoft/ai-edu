...
其中，$X^T$是X的转置矩阵，$X^{\top} X$一定是个方阵，并且假设其存在逆矩阵，把它移到等式右侧来：

$$W = (X^{\top} X)^{-1}{X^T Y} \tag{10}$$
...
$$J(w) = \sum (x_i w_i -y_i)^2=(XW - Y)^{\top} \cdot (XW - Y) \tag{12}$$
...
4. 再乘以 $Y$，$Y$是 $1000\times 1$ 的矩阵，所以最后变成 $3x1$ 的矩阵，成为 $W$ 的解，其中包括一个偏移值 $b$ 和两个权重值$w$，3个值在一个向量里。
