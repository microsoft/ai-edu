...
3. 候选隐藏状态
   ...
   $$
   \tilde{h}_ t = \tanh((r_t \circ h_{t-1}) \cdot W_h + x_t \cdot U_h)
   \tag{3}
   $$
...
4. 隐藏状态
   ...
   $$
   h = (1 - z_t) \circ h_{t-1} + z_t \circ \tilde{h}_ t
   \tag{4}
   $$
...
首先，上一时刻的状态$h_{t-1}$通过重置门，加上当前时刻输入信息，共同构成当前时刻的即时状态$\tilde{h}_ t$，并通过$\tanh$函数映射到$[-1,1]$区间。
...
$$
z_{\tilde{h}_ t} = (r_t \circ h_{t-1}) \cdot W_h + x_t \cdot U_h
\tag{7}
$$
...
\begin{aligned}
    ...
    &= \delta_t \cdot (-diag[h_{t-1}] + diag[\tilde{h}_ t]) \cdot diag[z_t \circ (1-z_t)] \\\\
    &= \delta_t \circ (\tilde{h}_ t - h_{t-1}) \circ z_t \circ (1-z_t)
\end{aligned}
\tag{8}
$$
...
$$
\begin{aligned}
    \delta_{z_{\tilde{h}t}} &= \frac{\partial{loss}}{\partial{h_t}} \cdot \frac{\partial{h_t}}{\partial{\tilde{h}_ t}} \cdot \frac{\partial{\tilde{h}_ t}}{\partial{z_{\tilde{h}_ t}}} \\\\
    &= \delta_t \cdot diag[z_t] \cdot diag[1-(\tilde{h}_ t)^2] \\\\
    &= \delta_t \circ z_t \circ (1-(\tilde{h}_ t)^2)
\end{aligned}
\tag{9}
$$
...
$$
\begin{aligned}
    \delta_{z_{rt}} &= \frac{\partial{loss}}{\partial{\tilde{h}_ t}} \cdot \frac{\partial{\tilde{h}_ t}}{\partial{z_{\tilde{h}_ t}}} \cdot \frac{\partial{z_{\tilde{h}_ t}}}{\partial{r_t}} \cdot \frac{\partial{r_t}}{\partial{z_{r_t}}} \\\\
    &= \delta_{z_{\tilde{h}t}} \cdot W_h^{\top} \cdot diag[h_{t-1}] \cdot diag[r_t \circ (1-r_t)] \\\\
    &= \delta_{z_{\tilde{h}t}} \cdot W_h^{\top} \circ h_{t-1} \circ r_t \circ (1-r_t)
\end{aligned}
\tag{10}
$$
...
$$
\begin{aligned}
    d_{W_{h,t}} = \frac{\partial{loss}}{\partial{z_{\tilde{h}_ t}}} \cdot \frac{\partial{z_{\tilde{h}_ t}}}{\partial{W_h}} = (r_t \circ h_{t-1})^{\top} \cdot \delta_{z_{\tilde{h}t}}
\end{aligned}
\tag{11}
$$
...
$$
\begin{aligned}
    d_{U_{h,t}} = \frac{\partial{loss}}{\partial{z_{\tilde{h}_ t}}} \cdot \frac{\partial{z_{\tilde{h}_ t}}}{\partial{U_h}} = x_t^{\top} \cdot \delta_{z_{\tilde{h}t}}
\end{aligned}
\tag{12}
$$
...
$$
\begin{aligned}
\delta_{h_{t-1}} = \frac{\partial{loss}}{\partial{h_{t-1}}} &= \frac{\partial{loss}}{\partial{h_t}} \cdot \frac{\partial{h_t}}{\partial{h_{t-1}}} + \frac{\partial{loss}}{\partial{z_{\tilde{h}_ t}}} \cdot \frac{\partial{z_{\tilde{h}_ t}}}{\partial{h_{t-1}}} \\\\
...
$$
\begin{aligned}
\delta_{x_t} &= \frac{\partial{loss}}{\partial{x_t}} = \frac{\partial{loss}}{\partial{z_{\tilde{h}_ t}}} \cdot \frac{\partial{z_{\tilde{h}_ t}}}{\partial{x_t}} \\\\
