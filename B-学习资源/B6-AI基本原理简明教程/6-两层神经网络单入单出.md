...
如上图所示，我们暂时忘记数据集是图片（二维数组）这个性质，而是把它们当作是一个1x1的向量，作为神经网络的数据输入。如果是想学习针对图片的处理模式，可以手动搭建一个卷积神经网络，请参考我们的后续博客。
...
def ForwardCalculation(x, dictWeights):
    ...
    return A2, dictCache
```
由于参数较多，所以我们用一个dictionary(dictWeights)来保存这些参数。
...
$$
\frac{\partial{Loss}}{\partial{Z2}} = \frac{\partial{(\frac{1}{2}(Z-Y)^2)}}{\partial{Z2}} = Z2-Y
$$
...
以上这些标准值如何得到呢？试了很多组合后得到的，这就是所谓调参的过程了。
