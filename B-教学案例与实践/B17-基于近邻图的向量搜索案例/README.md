...
另外，在选择划分值构建KD-Tree时，我们通常会选择方差最大的前k维的平均值作为划分值。因为方差越大，代表数据越分散，而选择数据比较分散的维度，会更容易将样本划分开。当然，划分的规则并不绝对，可以根据实际的需求而调整。

#### 构建算法

1. 从方差最大的前K个维度中随机选择一个维度作为划分维度，将中位数作为划分值，划分出两组子空间，小于等于划分值的归入左子树空间，大于划分值的归入右子树空间。
...
1. 随机划分一组子空间。可以采用不同的划分的方式，在SPTAG中采用了TPTree的方式划分。
...
如上图所示，第一次划分出一组子空间时，样本中包含了两个p点的真实最近邻，利用Brute-Force对该子空间构建KNN图后，P点会与这两个点连接；在第二次划分子空间的时候，样本包含了两个新的真实最近邻，构建KNN后，会使p点与新的真实最近邻相连。重复多次随机划分并对划分的子空间构建KNN图后，会使p点与大部分的真实最近邻连通，从而构建比较接近真实KNN图的近似KNN图。
