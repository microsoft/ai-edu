...
以做线性回归的实际值和预测值为例，若自变量x是：

$$\begin{pmatrix} -2, -1, 0, 1, 2 \end{pmatrix}$$

这样5个值，对应的期望值y是：

$$y=\begin{pmatrix} -3, 0, 0, 3, 4 \end{pmatrix}$$

这样的值，目前预测使用的参数是(w, b) = (2, 1)，即$y=2x+1$, 那么预测得到的值：

$$a = \begin{pmatrix}-3, -1, 1, 3, 5 \end{pmatrix}$$

采用均方误差计算这个预测和实际的损失就是：

$$Loss = \frac{1}{2}\sum_{i = 0}^{4}(a_i - y_i)^{2}=1.5$$

也就是3。那么如果采用的参量是(w,b)=(0,0)，即$y=0x+0=0$，预测出来的值是：

$$[0, 0, 0, 0, 0]$$

这是一个显然错误的预测结果，此时的损失大小就是34，$3 < 34$, 那么(2, 1)是一组比(0, 0)要合适的参量。
...
## MSE (均方误差函数)
  ...
  + 函数形式：
    ...
    $$loss = \frac{1}{2}\sum_{i}(a_i - y_i) ^ 2$$, 
    其中， $a$是网络预测所得到的结果，$y$代表期望得到的结果，也就是数据的标签，$i$是样本的序号。
  + 反向传播：

    $$\frac{\partial{loss}}{\partial{z}} = \sum_{i}(a_i - y_i)*\frac{\partial{a_i}}{\partial{z}}$$
  + 缺点:
    和$\frac{\partial{a_i}}{\partial{z}}$关系密切，可能会产生收敛速度缓慢的现象，以下图为例（激活函数为sigmoid）
