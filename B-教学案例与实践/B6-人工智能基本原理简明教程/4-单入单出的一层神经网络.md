...
def UpdateWeights(w, b, dW, dB, eta):
    ...
    return w,b
```
上面有两个求梯度函数，第一个用于数组数据（当输入的X/Y/Z都是数组时），第二个用于标量数据(x/y/z都是标量)。但二者但最后输出的dW/dB都是标量，因为只有一个神经元。
...
```
最终我们得到了W=1.846，B=3.085，然后根据这两个值画出了上图中的红线$y=1.846x+3.085$。可以看到，我们初步完成了任务，但是，红线似乎和蓝点的吻合程度还可以改进。如何改进呢？请看本章的《扩展阅读》部分吧！
