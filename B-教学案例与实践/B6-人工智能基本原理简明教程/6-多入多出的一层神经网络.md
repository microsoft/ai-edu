...
最后两组数据相比，从图示上看效果差不多，从数据上看0.01的W,B值要比0.1的W,B值小10倍左右，这是因为这个公式：$W=W-eta*dW$中dW的值，每次反馈叠加到W时，分别乘以0.1和0.01造成的10倍的差距。这样说明了另外一个问题：神经网络学习的结果并非是唯一的，很有可能因为参数不同而得到差别很大的两组权重值。但由于线性关系，两者都是正确的结果。
