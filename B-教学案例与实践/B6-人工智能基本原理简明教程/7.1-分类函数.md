...
上式中:
- $z_j$是对第 j 项的分类原始值
- $z_i$是参与分类计算的每个类别的原始值（矩阵运算的结果）
- m 是总的分类数
- $a_j$是对第 j 项的计算结果

也就是说把接收到的输入归一化成一个每个分量都在$(0,1)$之间并且总和为一的一个概率函数。

在上图中，$A1+A2+A3=1$。分类的方式是：看A1/A2/A3三者谁的比重最大，就算是那一类。比如A1=0.6, A2=0.1, A3=0.3，则这个样本就算是第一类了。
...
- $i=j$时，求$a_j$对$z_i$的导数，此时分子上的$e^{z_j}$要参与求导。参考基本数学导数公式33：

$$
\frac{\partial{a_j}}{\partial{z_i}} = \frac{\partial{(e^{z_j}/(e^{z_1} + \dots + e^{z_j} + \dots +e^{z_m}))}}{\partial{z_j}} = \frac{\partial{(e^{z_j}/\sum)}}{\partial{z_j}} \\
=\frac{e^{z_j}\sum-e^{z_j}e^{z_j}}{(\sum)^2} \\
=\frac{e^{z_j}}{\sum} - \frac{e^{z_i}e^{z_j}}{(\sum)^2} \\
= \frac{e^{z_j}}{\sum} - \frac{e^{z_i}}{\sum}\frac{e^{z_j}}{\sum}\\
= a_j - a_i*a_j = a_j-a^2_j=a_j(1-a_j) \tag{因为$a_i=a_j$}
$$

也就是说，此时softmax的梯度就是$a_j(1-a_j)$。

- $i \neq j$时，$a_j$对$z_i$的导数，分子上的$z_j与i$没有关系，不参与求导，分母的求和项中$e^{z_i}$要参与求导。同样是公式33，因为分子$e^{z_j}$对$e^{z_i}$求导的结果是0：
...
看上图，假设我们要求Loss值对Z1的偏导数，应该怎么做呢？从Softmax函数的形式来看，无论是A1，A2，A3，都是与Z1，Z2，Z3相关的，而不是一对一的关系，所以，想求Loss对Z1的偏导，必须把Loss->A1->Z1， Loss->A2->Z1，Loss->A3->Z1，这三者加起来。于是有了如下公式：

$$
\frac{\partial{Loss}}{\partial{z_i}}=\sum_j \frac{\partial{Loss}}{\partial{a_j}}\frac{\partial{a_j}}{\partial{z_i}}
$$
...
=-y_j + a_i*1=a_i-y_j
$$
因为$y_i$是个[1,0,0]或者[0,1,0]或者[0,0,1]的值，这三者加起来，就是[1,1,1]，在矩阵运算里相当于1，在乘法里相当于什么都不做。
