...
def BackPropagation(X, Y, dictCache, dictWeights, batch_size):
    A1 = dictCache["A1"]
    A2 = dictCache["A2"]
    W2 = dictWeights["W2"]

    dLoss_Z2 = (A2 - Y)
    dW2 = np.dot(dLoss_Z2, A1.T) / batch_size
    dB2 = np.sum(dLoss_Z2, axis=1, keepdims=True) / batch_size

    dLoss_A1 = np.dot(W2.T, dLoss_Z2)
    dA1_Z1 = A1 * (1 - A1)
    
    dLoss_Z1 = dLoss_A1 * dA1_Z1
    dW1 = np.dot(dLoss_Z1, X.T) / batch_size
    dB1 = np.sum(dLoss_Z1, axis=1, keepdims=True) / batch_size
...
def ShowResult(iteration, neuron):
    # draw train data
    ...
    plt.title(str.format("iteraion={0},neuron={1}", iteration, neuron))
    plt.show()
...
batch_size = 10
loop = int(num_samples/batch_size)
for iteration in range(max_iteration):
    start = 0
    for i in range(loop):
        end = start + batch_size
        #print(i, start, end)
        x = X[0,start:end].reshape(1,batch_size)
        y = Y[0,start:end].reshape(1,batch_size)
        A2, dictCache = ForwardCalculation(x, dictWeights)
        dictGrads = BackPropagation(x, y, dictCache, dictWeights, batch_size)
