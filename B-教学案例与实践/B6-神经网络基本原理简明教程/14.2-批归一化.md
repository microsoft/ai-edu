...
比如，在上图中，假设X是服从蓝色曲线的分布，经过上述公式后，有可能变成了绿色曲线的分布。

标准正态分布的数值密度占比如下图所示：

<img src=".\Images\14\bn2.png">

有68%的值落在[-1,1]之间，有95%的值落在[-2,2]之间。再回忆一下激活函数Sigmoid的图像，在[-2,2]之外的区域，其导数值很小（小于0.1），导致反向传播的力度很小。
...
为什么要有Batch Normalization呢？这就会有一种什么问题呢？
...
$$
\mu = {1 \over m}\sum_1^m x_i \tag{1}
$$

$$
\sigma = {1 \over m} \sum_1^m (x_i-\mu)^2 \tag{2}
$$

$$
n_i = {x_i-\mu \over \sqrt{\sigma + \epsilon}} \tag{3}
$$

$$
z_i = \gamma n_i + \beta \tag{4}
$$
