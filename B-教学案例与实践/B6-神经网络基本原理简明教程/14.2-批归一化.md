...
|符号|数据类型|数据形状|
|:---------:|:-----------:|:---------:|
|$X$| 输入数据矩阵 | [batchSize, Channles] |
|$x_i$|输入数据第i个样本| [1, Channels] |
|$N$| 经过归一化的数据矩阵 | [batchSize, Channles] |
|$n_i$| 经过归一化的单样本 | [1, Channles] |
|$\mu_B$| 批数据均值 | [1, Channels] |
|$\sigma^2_B$| 批数据方差 | [1, Channels] |
|$m$|批样本数量| [1] |
|$\gamma$|线性变换参数| [1, Channels] |
|$\beta$|线性变换参数| [1, Channels] |
|$Z$|线性变换后的矩阵| [1, Channels] |
|$z_i$|线性变换后的单样本| [1, Channels] |
|$\delta$| 反向传入的误差 | [batchSize, Channles] |
...
其中，$\gamma 和 \beta$是训练出来的，$\epsilon$是防止$\mu_B^2$为0时加的一个很小的数值，通常为1e-6。
...
$$
{dJ \over d n_i}=  {dJ \over dz_i}{dz_i \over dn_i} = \delta \cdot \gamma\tag{14}
$$
...
$$
$$
=\sum_{i=1}^m {dJ \over d n_i} \cdot {-1 \over 2}(x_i-\mu_B)(\sigma^2_B + \epsilon)^{-3/2} \tag{16}
$$
