...
首先，只使用1000个样本来做训练（原始数据集有60000个样本），如下面的代码所示，调用一个ReadLessData(1000)函数，并且用GenerateDevSet(k=10)函数把1000个样本分成900和100两部分，分别做为训练集和验证集：
...
net.train(dataReader, checkpoint=1)函数的参数checkpoint的含义是，每隔1个epoch记录一次训练过程中的损失值和准确率。
...
loss_train=0.0007, accuracy_train=1.000000
loss_valid=1.0068, accuracy_valid=0.830000
epoch=198, total_iteration=1782
loss_train=0.0015, accuracy_train=1.000000
loss_valid=1.0131, accuracy_valid=0.830000
epoch=199, total_iteration=1791
loss_train=0.0008, accuracy_train=1.000000
loss_valid=1.0098, accuracy_valid=0.830000
epoch=199, total_iteration=1799
loss_train=0.0010, accuracy_train=1.000000
loss_valid=1.0090, accuracy_valid=0.830000
time used: 2.3161489963531494
total weights abs sum= 1719.3020773030603
total weights = 26520
little weights = 2810
zero weights = 25
```
...
结果说明：
1. 第199个epoch上（从0开始计数，所以实际是第200个epoch），训练集的损失为0.0010，准确率为100%。测试集损失值1.0090，准确率83%。过拟合线性很严重。
3. total weights abs sum = 3721.4，实际上是把所有全连接层的权重值先取绝对值，再求和。这个值和下面三个值在后面会有比较说明。
4. total weights = 26520，一共26520个权重值。
5. little weights = 2810，一共2810个权重值小于0.01。
6. zero weights = 25，是权重值中接近于0的数量（小于0.0001）。

另外，每次运行的结果都会有微小差异，并不会影响对问题的性质的分析。
...
2. 特征工程
    ...
    在传统机器学习中，特征的选择很重要，都是由人来完成。在神经网络中，特征工程的重要程度变低了。

3. 简化模型
   
    本来需要2层神经网络能解决的问题，就不需要3层。
...
5. 正则 Regularization
    ...
    包括L1和L2正则，通过在损失函数后增加惩罚项，降低权重参数拟合度。

6. 丢弃法 dropout
   
    在训练过程中，随机地禁止一些神经元的工作，阻断不良特征的蔓延。
...
1. 本小节内搭建的深度网络一共有多少个参数？
   - 第一层：w=30*784,b=784
   - 第二/三/四层：w=30*30,b=30
   - 第五层：w=10*30,b=10
      
    所以一共是25544个参数。

2. 为什么total_iteration=1799呢？
   因为batch size=100，1000个样本中的100个做为验证集，其余900个训练样本，一共200个epoch，900/100*200=1800，从0计数的话就是1799。
