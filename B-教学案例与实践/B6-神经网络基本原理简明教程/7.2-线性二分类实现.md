...
由于北京的特殊地理位置，人口密集，城市拥挤，空气污染问题一直被人们所关注。下面这组数据，是不同温度、高度下的PM2.5（可入肺颗粒物，标签为1）、PM10（可吸入颗粒物，标签为2）在地表人类活动层的主要占比。
...
### 问题，给定温度高度条件下，预测当前那种污染物占主要比重？
1. 温度=5.1摄氏度，高度=12.7
2. 温度=25.3摄氏度，高度=42.1
3. 温度=34.3摄氏度，高度=82.2
...
- 输入特征是温度和高度，所以我们在输入层设置两个输入X1=温度值，X2=高度值
...
$X_m$表示第m个样本值，$x_{m,n}$表示第m个样本的第n个特征值。样本数据集中一共有119个数据，每个数据有两个特征：温度和高度。所以定义矩阵如下：
$$
X = 
\begin{pmatrix}
X_1 & X_2 \dots X_{119}
...
def ToBool(YData):
    ...
    return Y
```
遍历标签数据YData中所有记录，设置类别为1的标签为负例0，设置类别为2的标签为正例1。
...
1. 温度=5.1摄氏度，高度=12.7，概率0.006，第一类
2. 温度=25.3摄氏度，高度=42.1，概率0.316，第一类
3. 温度=34.3摄氏度，高度=82.2，概率0.997，第二类
