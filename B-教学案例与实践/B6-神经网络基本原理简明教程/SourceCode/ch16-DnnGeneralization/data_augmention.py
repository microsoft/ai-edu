...
if __name__=="__main__":
    ...
    index_list = random.sample(range(60000), number_image)
    image = mnist_image_reader(data_path, index_list)
    image_list = [image[i, :, :] for i in range(number_image)]
    label = mnist_label_reader(label_path, index_list)

    rotated_image_10 = map(lambda x: rotate(x, 10), image_list)
    rotated_image_minus_10 = map(lambda x: rotate(x, -10), image_list)

    stretch_image_11_v = map(lambda x: stretch(x, 1.1), image_list)
    stretch_image_09_v = map(lambda x: stretch(x, 0.9), image_list)
    stretch_image_11_h = map(lambda x: stretch(x, 1.1, 1), image_list)
    stretch_image_09_h = map(lambda x: stretch(x, 0.9, 1), image_list)

    translate_image_2_v = map(lambda x: translate(x, 2), image_list)
    ...
    np.savez("data.npz", data=all_image, label=all_label)
    """
