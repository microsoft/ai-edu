...
在上一节中，我们知道了BN的正向计算过程，这一节中，为了实现完整的BN层，我们首先需要推导它的反向传播公式，然后用代码实现。此节中的公式序号解上一小节，以便于说明。
...
可以看到到达相同的损失值0.26时，只是用了不到2个epoch的1273个迭代，相比14.6中的29个epoch，速度提升了20多倍，时间用了以前的十分之一。这是因为BN的正向和反向计算过程还是比较复杂的，需要花费一些时间。
