...
<img src="..\Images\16\L1_sin_result.png">
...
|比较项|无正则项|L2|L1|
|---|---|---|---|
|代价函数|$J(w,b)$|$J(w,b)+\lambda \lvert w \rvert^2_2$|$J(w,b)+\lambda \lvert w \rvert_1$|
|梯度计算|$dw$|$dw+\lambda \cdot w$|$dw+\lambda \cdot sign \lvert w \rvert$|
|准确率|0.961|0.982|0.987||
|总参数数量|544|544|544|
|参数值小于1e-2的数量|7|204|524|
|参数值小于1e-4的数量|0|196|492|
|第1层Norm1|8.66|6.84|4.09|
|第2层Norm1|104.26|34.44|6.38|
|第3层Norm1|97.74|18.96|6.73|
|第4层Norm1|9.03|4.22|4.41|
|第1层Norm2|2.31|1.71|1.71|
|第2层Norm2|6.81|2.15|2.23|
|第3层Norm2|5.51|2.45|2.81|
|第4层Norm2|2.78|2.13|2.59|

通过比较各层的权重值的第一范数值Norm1，可以看到L1的最小，因为L1正则的效果，就是让参数稀疏化，对非主流特征不敏感，以形成特征选择。用通俗的话讲就是权重值矩阵中很多为项0或者接近0。

这一点从参数值小于1e-4的数量中也可以看出来，L1达到了492个，一共才有544个参数，90%的参数都是很小的数。
...
1. 尝试改变代码中$\lambda$的数值，看看最后的拟合结果及准确率有何变化。
