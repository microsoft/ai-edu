...
公式4中的分类函数C和损失函数L因不同的网络类型而不同，如表19-5所示。

$$
J = \frac{1}{T} \sum_t^Tloss_t \tag{6}
$$
...
从公式12可以看到，求任意时间步t的$dh_t$是关键的环节，有了它之后，后面的问题都和全连接网络一样了，在19.1和19.2节中也有讲述具体的方法，在此不再赘述。求$dh_t$时，是要依赖$dh_{t+1}$的结果的，所以，在通过时间的反向传播时，要先计算最后一个时间步的$dh_T$，然后按照时间倒流的顺序一步步向前推导。
