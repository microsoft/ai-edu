...
class PoolingLayer(CLayer):
   def __init__(self, input_c, input_h, input_w, pool_h, pool_w, stride, pooling_type):
        self.num_input_channel = input_c
        self.input_height = input_h
        self.input_width = input_w
        self.pool_height = pool_h
        self.pool_width = pool_w
        self.stride = stride
    ...
    def forward(self, x):
        ...
        self.z = np.zeros((batch_size, self.num_input_channel, self.output_height, self.output_width))
            ...
            for c in range(self.num_input_channel):
                for i in range(self.output_height):
                    ...
                    for j in range(self.output_width):
                        ...
                        target_array = self.x[c,i_start:i_end, j_start:j_end]
            ...
            for c in range(self.num_input_channel):
                for i in range(self.output_height):
                    ...
                    for j in range(self.output_width):
                        j_start = j * self.stride
                        j_end = j_start + self.pool_width
                        if self.mode == PoolingTypes.MAX:
                            m,n = PoolingLayer.get_max_index(self.x[c], i_start, i_end, j_start, j_end)
                            delta_out[b,c,m,n] = delta_in[b,c,i,j]
                        elif self.mode == PoolingTypes.MEAN:
