...
- 右图
  ...
  倾斜的分界线及其分类间隔。与中图比较可以看到，中图的分类间隔的宽度大于右侧的分类间隔，而且是所有可能的情况中的最大值，属于最优解。
...
有些读者会有疑问：为什么公式 5 和 6 的值正好是 $+1$ 和 $-1$ 吗？如果间隔变窄了，该等式还成立吗？
...
其中：$A_1=1，B_1=1，C_1=-4$，那么 $p_2(3,3)$ 到分割线的距离是：

$$
d_1' = \frac{|A_1x_{1}+B_1x_{2}+C_1|}{\sqrt{A_1^2+B_1^2}} = \frac{|1 \times 3 + 1 \times 3 -4|}{\sqrt{1^2+1^2}}=\frac{2}{\sqrt{2}}，（=\sqrt{2}）
$$

对于公式 7，我们两边都除以 2，得到公式 8，该直线位置不变：

$$
0.5x_1 + 0.5x_2 - 2=0 \tag{8}
$$

此时，$A_2=0.5，B_2=0.5，C_2=-2$，再计算 $p_2$ 到分割线的距离：

$$
d_1'' = \frac{|A_2x_{1}+B_2x_{2}+C|}{\sqrt{A_2^2+B_2^2}} = \frac{|0.5 \times 3 + 0.5 \times 3 -2|}{\sqrt{0.5^2+0.5^2}}=\frac{1}{\sqrt{0.5}}，（=\sqrt{2}）
$$

公式 7 和 8 两个方程表示了一条直线，点 $p_2$ 到分割线的距离都是 $\sqrt{2}$，但是 $|Ax_{1}+Bx_{2}+C|$ 的值并不相同，前者为 2，后者为 1。

这就说明，无论分割线的位置如何，我们总可以改变 $A、B$ 的值让公式 5 和 6 成立，因为 $\sqrt{A^2+B^2}$ 也会做相应的变化，令最终的距离值（本例中为 $\sqrt{2}$）不会变化。

所以，分类间隔 $d$ 可以表示为：

$$
d = d_1 + d_2 = \frac{|Ax_{11}+Bx_{12}+C|}{\sqrt{A^2+B^2}}+\frac{|Ax_{21}+Bx_{22}+C|}{\sqrt{A^2+B^2}}=\frac{1}{\sqrt{A^2+B^2}}+\frac{1}{\sqrt{A^2+B^2}}=\frac{2}{\sqrt{A^2+B^2}}
$$
...
其中：$\boldsymbol{w} = (A \quad B)，\boldsymbol{x} = (x_1 \quad x_2)^T$，则公式 3 可以写成：

$$
d = \frac{|\boldsymbol{w} \boldsymbol{x}+C|}{\sqrt{w_1^2 + w_2^2}}=\frac{|\boldsymbol{w}\boldsymbol{x}+C|}{||\boldsymbol{w}||} \tag{6}
$$
...
$$
d_1 = \frac{|Ax_{11}+Bx_{12}+C|}{\sqrt{A^2+B^2}} = \frac{|\boldsymbol{w}\boldsymbol{x_1}+C|}{||\boldsymbol{w}||} \tag{7}
$$

同理，点 $p_2(x_{21},x_{22})$ 到分界线的距离是：

$$
d_2 = \frac{|Ax_{21}+Bx_{22}+C|}{\sqrt{A^2+B^2}} = \frac{|\boldsymbol{w}\boldsymbol{x_2}+C|}{||\boldsymbol{w}||}  \tag{8}
$$
