...
在图 5 的中图，分界线公式 $Ax_1+Bx_2+C=0$，是大家都可以理解的。我们还定义了两侧的两条间隔边界的直线方程：
...
其中：$A_1=1，B_1=1，C_1=-4$，那么 $p_2(3,3)$ 到分界线的距离是：

$$
d' = \frac{|1 \times 3 + 1 \times 3 -4|}{\sqrt{1^2+1^2}}=\frac{2}{\sqrt{2}}，（=\sqrt{2}）
$$

对于公式 8，我们两边都除以 2，得到公式 9，该直线位置不变：
...
公式 8 和 9 表示了同一条直线，点 $p_2$ 到其的距离都是 $\sqrt{2}$，但是分子 $|Ax_{1}+Bx_{2}+C|$ 的值并不相同，前者为 2，后者为 1。这就说明，无论分割线的位置如何，我们总可以改变 $A、B、C$ 的值让公式 5 和 6 成立，因为分母 $\sqrt{A^2+B^2}$ 也会做相应的变化，令最终的距离值（本例中为 $\sqrt{2}$）不会变化。

我们把正类样本点 $p_2(3,3)$ 的坐标值代入公式 9：

$$
Ax_{1}+Bx_{2}+C=0.5 \times 3 + 0.5 \times 3 - 2 = +1
$$

再把负类样本点 $p_1(1,1)$ 的坐标值代入公式 9：

$$
Ax_{1}+Bx_{2}+C=0.5 \times 1 + 0.5 \times 1 - 2 = -1
$$

可以看到上面两式和公式 5、6 正好吻合，证明了公式 5、6 的可行性。

所以，图 5 的中图中的分类间隔 $d$ 可以表示为：
...
- 右图
    ...
    重新计算的分界线及分类间隔的结果，可以想象成是原有的分类间隔在平面上逆时针旋转了一个角度。可以看到，负类中有两个点在分类间隔上，这并不是巧合，而是分类间隔在这个角度上，可以刚好分类正确，并且得到的 $d_2$ 值 最大。这一点在很多文字资料中没有讲清楚，造成很多读者误以为支持向量一定是一个正类样本，两个负类样本，或者相反。
...
公式 15 的几何意义很明确，即所有正类样本都在分类间隔之上，而所有负类样本都在分类间隔之下。
...
$$
y_i(\boldsymbol{w}\boldsymbol{x_i}+C) \ge 1 \tag{16}
$$

在图 5 右子图中，$p_1、p_2、p_4$ 处于分类间隔边界上，叫做**支持向量**，因为它们限制了间隔的宽度。在二维以上的数据都可以叫做向量。

### 优化目标函数

到此为止，我们可以总结出分类间隔的基本条件，公式 4 中的 $d$ 用公式 14 代替：
...
\begin{aligned}
    &\min \frac{1}{||w||^2}
    \\\\
    s.t. \quad & y_i(\boldsymbol{w} \boldsymbol{x_i}+C) \ge 1
\end{aligned}
\tag{19}
$$
