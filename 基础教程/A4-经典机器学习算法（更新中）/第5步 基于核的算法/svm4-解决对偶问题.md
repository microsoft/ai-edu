...
因为这个例子比较简单，所以可以直接分别对 $L$ 求 $x,\alpha$ 的偏导，并令结果为 0，具体过程不再赘述，最终得到：
...
```
表 5.4.1 的一行数据，就代表图 5.4.1 中的一条黄色竖线，是在给定 $x$ 时，$\alpha$ 分别取值为 [0, 0.5, 1, 1.5, 2] 得到的 $L$ 值。
...
$$
P(x)=\underset{\alpha}{\max} \ L(x,\alpha)=\underset{\alpha}{\max} [f(x)+\alpha g(x)]=
\begin{cases}
    f(x), \quad x \in 限定区域内
    \\\\
...
min_x = np.min(max_a)
```
就是在表 5.4.1 中的最后一列中，取最小值，就得到 $p^*$。最后得到的打印输出是：
