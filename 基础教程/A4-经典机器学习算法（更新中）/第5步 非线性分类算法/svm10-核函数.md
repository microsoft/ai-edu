...
我们先看一个简单的例子，假设有 7 个样本，其中 4 个正类样本 $x_{1}=[-1.5,\ -1,\ 1,\ 1.5]$，3 个负类样本 $x_{1}=[-0.5,\ 0,\ 0.5]$，它们都排列在 $x_{1}$ 轴上（即 $x_{2}=0$），如图 5.10.1 的左子图所示。

<img src="./images/5-10-1.png" />

<center>图 5.10.1 </center>

我们无法将这两类样本点做线性分割。但是，如同前面学习的多项式回归一样，如果令 $x_{2}=x_{1}^2$，则可以得到表 5.10.1 中的数据，也就是说，把一维的特征无形中增加了一维，变成二维的了。
...
把表 5.10.1 的新生成的样本数据绘制到图 5.10.1 的中子图，得到一条抛物线，可以明显看出用一条红色的水平虚线就能够轻松地分开两类样本点。
...
```
生成的 100 个样本，取出前 10 个列在表 5.10.2 的 $x_{1}、x_{2}、y$ 三列，其中，$x_{1}、x_{2}$ 可以理解为横纵坐标，$y$ 是标签值，$x_{3}$ 列在后面的内容中学习。
...
用前两行代码获得正类（positive）和负类（negative）的列表，假设是 [1, 0, 0, 1, ...] 的话，pos=[True, False, False, True, ...]，neg=[False, True, True, False, ...]。然后在后面的 x[pos, 0] 将会返回 pos 为 True 的行的第 0 列，x[neg, 1] 将会返回 pos 为 False 的行的第 1 列。

后面几行代码绘制正类样本为圆点，负类样本为三角。这里吐槽一下 sklearn 的函数，居然把 inner（内圆） 样本作为正类，而把 outer（外圆）样本作为负类，这样会造成“负类样本大于正类样本”的假象，不利于读者理解。

最后画出图 5.10.2 的左子图。

<img src="./images/5-10-2.png" />

<center>图 5.10.2 </center>

通过观察左子图的样本分布，可以知道是线性不可分的，但是直观上看可以用一个圆形来分开两类样本点之间，根据圆的方程：$r^2=x^2+y^2$，我们试试给样本增加一个维度：$x_{3} = x_{1}^2 + x_{2}^2$，于是得到了表 5.10.2 的最后一列特征值。
...
然后在三维坐标系中绘制新的样本分布，调整观察角度后得到图 5.10.2 的中子图和右子图：
