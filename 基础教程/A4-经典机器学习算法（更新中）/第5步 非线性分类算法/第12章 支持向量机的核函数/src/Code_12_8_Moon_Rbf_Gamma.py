...
# 显示等高线分类结果
def show_contour_result(ax, model, X, Y, scope):   
    set_ax(ax, scope)
    ...
    ax.contour(P, Q, y_pred, cmap=cm.coolwarm)
    #ax.plot_surface(P, Q, y_pred, cmap=cm.coolwarm)
    # 绘制原始样本点用于比对
    if (X.shape[0]<=10):
        draw_2d_samples(ax, X, Y, display_text=True)
    else:
...
def show_3_contour(X, Y, gamma):
    ...
    for i in range(3):
        ...
        ax = plt.subplot(1,3,i+1)
        ax.set_title(gamma_str)
        show_contour_result(ax, model, X, Y, scope)
