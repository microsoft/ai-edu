...
```
------ 状态价值函数 -----
[[0.014 0.012 0.021 0.008]
 [0.018 0.    0.043 0.   ]
 [0.039 0.101 0.154 0.   ]
 [0.    0.187 0.456 0.   ]]
误差 = 0.006852650015689751
```

注意，每次运行得到循环幕数都会不一样，无法控制。

如果运行 10 次，每次循环 30000 幕，每 1000 次循环记录一次当前的 V 值，并与基准值计算 RMSE 误差，并把这些误差求 10 次平均的话，会得到图 10.4.2。
...
可以看到误差情况不很平稳，如果只看单次运行的误差将会更糟糕，这也是蒙特卡洛法的一个特点，不经过几万次的循环，无法得到比较准确的估算值。

如果想得到比较准确的值，可以多次运行后得到不同的 V 值，然后求这些 V 值的平均值。
