...
\\
&= \frac{\partial \pi(a)}{\partial Q(a)}R_a + \frac{\pi(b)}{\partial Q(a)}R_b+ \frac{\pi (c)}{\partial Q(a)}R_c
\\
&=\pi(a)(1-\pi(a))R_a-\pi(a)\pi(b)R_b-\pi(a)\pi(c)R_c
\\
&=\pi(a)[R_a - (\pi(a)R_a+\pi(a)R_b+\pi(c)R_c)]
\\
...
由于是假设执行了动作 $a$，所以 $R_a$ 就是当前动作的收益 $R_t$；$\mathbb E[R_t]$ 可以用当前的收益的均值来表示，简写为 $\bar{R}$。则式（2.5.7）进一步表示为 $\pi(a)(R_t-\bar{R})$，那么最终结果是：

$$
Q_{t+1}(a) = Q_t(a) + \alpha (R_t - \bar R)\pi(a) \tag{2.5.8}
$$


可以表示为在 $t$ 时刻获得的收益的期望值减去到目前为止收益的平均值，再乘以对 $\pi(x)$ 求导的结果：

$$
\nabla f(a) = (R_t - \bar{R})(1_{a=x}-\pi(x)) \tag{2.5.9}
$$
