...
\\
&=\pi(a)\big[R_a - [\pi(a)R_a+\pi(a)R_b+\pi(c)R_c]\big]=\pi(a)(R_a-\sum_x \pi_x R_x)\\
...
$$
\begin{cases}
Q_t(a) \doteq Q_{t-1} (a) + \alpha (R_t - \bar{R})(1-\pi(a)), & 被选动作 a=x
\\
Q_t(x) \doteq Q_{t-1} (x) + \alpha (R_t - \bar{R})(-\pi(x)), & 其它动作 a \ne x
\end{cases}
\tag{2.5.9}
$$
...
右图是 Softmax 函数计算的概率，大概在第 5 步时，动作 2 就已经脱颖而出了，然后一路飙升。在任意时刻，所有的动作的概率之和为 1，这是由 softmax 的算法决定的。

打印输出最终的动作价值和备选概率，可以看到，基本上有很少的几率可以选到 0 号和 1 号动作，99.99% 都会选择 2 号动作。

```
最终的动作价值 = [-0.48 -0.27  9.87]
最终的备选概率 = [3.19530774e-05 3.94321840e-05 9.99928615e-01]
备选概率的和 = 1.0
```
