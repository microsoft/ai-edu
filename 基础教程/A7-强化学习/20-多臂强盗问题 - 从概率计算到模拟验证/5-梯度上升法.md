...
为了节省篇幅，还有一些绘图代码省略了，绘出的图如图 2.5.2 所示。

<center>
<img src='./img/algo-Gradient-P.png'/>

图 2.5.2 迭代 200 步内的备选动作概率的变化
（左图：动作价值的变化；右图：备选概率的变化）
</center>

从左图看，10 个动作的初始值都是 0，随着算法的进行而分裂，第 9 个动作（从 0 开始数）最后可以到达 4，其它 9 个动作按照它们的收益均值依次排列。但是在任意时刻，它们的总和都是 0，这是由算法决定的。

```
最终的动作价值 = [-1.73 -1.08 -1.4  -0.8  -0.51 -0.13 -0.66  0.65  1.21  4.44]
动作价值的和 = 0.0
```

右图是 Softmax 函数计算的概率，第 50 步时，动作 9 就已经多颖而出了，然后一路飙升。所有的动作的概率之和为 1。
