...
在本节中，我们以代码的生命周期为例，通过描述开发（开发、调试、审查、签入）的状态/过程，来分析解决上述问题。


### 2 建立模型


图 1 是一个有关代码生命周期的马尔可夫链，也可以叫做状态转移图，并且给每个状态都赋予了一个即时奖励值。
...
- R：审查
  ...
  由小组中其他人做代码审查。
  - 有 0.9 的概率进入代码复审状态。
  - 有 0.1 的概率，发现代码质量根本不行，直接扔掉，进入结束状态。
...
- M：签入
  ...
  代码签入到 main branch，然后结束。

- E：结束
  
  结束本次代码的生命周期。进入此状态后将不再进行转移，或者是说以 100% 的概率转移到自己，叫做结束状态或者吸收状态。
...
|P: 从$\rightarrow$到|缺陷|编码|审查|复审|签入|重构|结束|
|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|
|**缺陷**|0.7|0.3|-|-|-|-|-|
|**编码**|0.6|-|0.4|-|-|-|-|
|**审查**|-|-|-|0.9|-|-|0.1|
...
- 在 $s_a$ 状态得到 $R(s)$ 的表达式。
  在使用**注重过程**的奖励函数定义方式时，如果从$s_a$ 转移到 $s_b$ 或 $s_c$ 的过程中，分别可以得到 $r_1,r_2$ 的奖励，则 $s_a$ 的奖励函数定义为一种期望：$R(s_a)=\mathbb E[R_{t+1}|S_t=s_a] = p_1 \cdot r_1+p_2 \cdot r_2$。
