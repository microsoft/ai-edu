...
if __name__=="__main__":
    ...
    for alpha in alphas:
        mean_errors = MultipleProcess(repeat, algoTD.TD_0, ds.Data_Frozen_Lake(), ds.States.Start, episodes, alpha, gamma, ground_truth, checkpoint)
        plt.plot(mean_errors, label="TD_0 - Start from 0 - " + str(alpha))

        mean_errors = MultipleProcess(repeat, algoTD.TD_0, ds.Data_Frozen_Lake(), None, episodes, alpha, gamma, ground_truth, checkpoint)
        plt.plot(mean_errors, label="TD_0 - Random Start - " + str(alpha), linestyle="--")
